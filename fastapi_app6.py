...
def process_data_into_df(data: List[Dict[str, Any]]) -> pd.DataFrame:
    """Helper to process raw nutrition data into a clean DataFrame with searchable text."""
    ...
    if not df.empty:
        ...
        df['searchable_text'] = (
            df['Dish Name'].astype(str).str.lower() + ' ' +
...
def load_nutrition_dataset(file_path: str = "nutrition_data.json"):
    ...
    try:
        ...
        nutrition_df = process_data_into_df(nutrition_data)
...
def create_fallback_nutrition_data():
    ...
    nutrition_data = fallback_data
    nutrition_df = process_data_into_df(fallback_data)
...
def download_and_extract_db_for_app():
    ...
    extract_path = os.path.join(temp_dir, "chroma_db")
...
def setup_vector_database(chroma_db_directory: Optional[str] = None, in_memory: bool = False):
    """
    Initializes Chroma vector database using Gemini embeddings.
    """
    try:
        ...
        if not chroma_db_directory:
             chroma_db_directory = os.path.join(tempfile.gettempdir(), "chroma_db")
        persist_path = None if in_memory else chroma_db_directory
...
if not FASTAPI_SECRET_KEY:
    logging.warning("⚠️ FASTAPI_SECRET_KEY not set in environment. Using insecure default for dev/test only.")
    FASTAPI_SECRET_KEY = "insecure-dev-key-do-not-use-in-prod"
...
async def startup_event():
    ...
    try:
        ...
        # Switched to Groq for Chat due to Gemini API issues
        if not GROQ_API_KEY:
             raise EnvironmentError("GROQ_API_KEY is not set.")

        logging.info(f"🔑 Using GROQ_API_KEY for LLMs")
        
        # Switched to Groq due to Gemini API issues
        if not GROQ_API_KEY:
             raise EnvironmentError("GROQ_API_KEY is not set.")
        ...
        logging.info("✅ Groq LLMs initialized.")
    except Exception as e:
    ...
    try:
        download_and_extract_db_for_app()
        db, _ = setup_vector_database(chroma_db_directory=os.path.join(tempfile.gettempdir(), "chroma_db"))
        logging.info("✅ Vector DB initialized.")
    except Exception as e:
        logging.error(f"❌ Vector DB init error: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Vector DB initialization failed.")
        ...
        def parse_agent_action_output(llm_output: Union[AIMessage, str]) -> AgentAction:
            ...
            try:
                ...
                if isinstance(data, dict):
                    ...
                    # Fallback strategies would go here
            except (ValidationError, json.JSONDecodeError) as e:
                logging.warning(f"JSON parsing failed: {e}. Raw content: {content_str[:100]}...")

            # Fallback: Check if the model just outputted a final answer or a tool name
            if isinstance(data, dict):
                 if "tool_name" not in data and "final_answer" not in data:
                      # Assume it is a final answer if no tool is specified
                       return AgentAction(thought="No tool specified, assuming final answer.", final_answer=content_str)
            
            # If all else fails and the model outputted text, treat it as a final answer
            return AgentAction(
                thought="Failed to parse structured output, using raw content as final answer.",
...
async def chat(chat_request: ChatRequest, request: Request):
    ...
    try:
        ...
        logging.error(f"❌ Global error in /chat endpoint for session {session_id}: {e}", exc_info=True)
        response_text = f"I'm experiencing a technical issue: {str(e)}. Please check the logs."
...
if __name__ == "__main__":
    ...
    logging.info("🚀 Starting Enhanced FastAPI application with Nutrition Database and Meal Analyzer...")
    # The test script uses port 10000, so we default to it here.
    uvicorn.run("fastapi_app6:app", host="0.0.0.0", port=int(os.getenv("PORT", 10000)), reload=False)
