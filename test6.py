...
try:
    ...
    qa_chain = RetrievalQA.from_chain_type(
        ...
        input_key="question", # Explicitly set the input key to 'question' to match the prompt and RunnableWithMessageHistory
        ...
        with st.chat_message("assistant"):
            with st.spinner("Thinking..."):
                ...
                try:
                    ...
                    # --- NEW DEBUGGING OUTPUT START ---
                    st.write("--- RAG Chain Result Debugging ---")
                    st.write(f"Type of result from conversational_qa_chain: {type(result)}")
                    st.write(f"Full result from conversational_qa_chain: {result}")
                    # --- NEW DEBUGGING OUTPUT END ---
                ...
                st.write("--- Overall Debugging Info ---")
                st.write(f"RAG Answer Status: {'Success' if rag_success else 'Failed'}")
                st.write(f"RAG Answer Content: {rag_answer[:500]}...") # Show first 500 chars
                ...
                # --- DEBUGGING OUTPUT START ---
                st.write(f"LLaMA Suggestion: {llama_suggestion[:200]}...")
                st.write(f"Mixtral Suggestion: {mixtral_suggestion[:200]}...")
                st.write(f"Gemma Suggestion: {gemma_suggestion[:200]}...")
                # --- DEBUGGING OUTPUT END ---
                ...
                try:
                    ...
                    # --- DEBUGGING OUTPUT START ---
                    st.write(f"Final Merge Prompt (first 1000 chars): {final_prompt_text[:1000]}...")
                    # --- DEBUGGING OUTPUT END ---
