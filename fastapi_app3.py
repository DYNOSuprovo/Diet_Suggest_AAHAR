# fastapi_app.py - Enhanced Agentic Version with JSON Nutrition Dataset Integration
import os
import json
import logging
import zipfile
...
def search_nutrition_data(query: str, limit: int = 5) -> List[Dict[str, Any]]:
    """
    Search the nutrition dataset using fuzzy matching for food items.
    """
    ...
    direct_matches = nutrition_df[
        nutrition_df['Dish Name'].str.lower().str.contains(query_lower, na=False)
    ]
    
    if not direct_matches.empty:
        return direct_matches.head(limit).to_dict('records')
    
    # Fuzzy matching on searchable text
    try:
        choices = nutrition_df['searchable_text'].tolist()
        matches = process.extract(query_lower, choices, limit=limit, scorer=fuzz.partial_ratio)
    ...
    category_matches = nutrition_df[
        nutrition_df['Category'].str.lower().str.contains(query_lower, na=False)
    ]
...
def get_regional_nutrition_suggestions(region: str, dietary_type: str, goal: str) -> List[Dict[str, Any]]:
    ...
    if region and region != "Indian":
        region_matches = filtered_df[
            filtered_df['Region'].str.lower().str.contains(region.lower(), na=False)
        ]
    ...
    if dietary_type == "vegan":
        ...
        filtered_df = filtered_df[
            filtered_df['Category'].str.contains('|'.join(vegan_categories), na=False)
        ]
        ...
        for keyword in non_veg_keywords:
            filtered_df = filtered_df[
                ~filtered_df['Dish Name'].str.lower().str.contains(keyword, na=False)
            ]
...
try:
    ...
    logging.warning(f"⚠️ Google Sheets connection failed: {e}. Logging disabled.", exc_info=True)

# FastAPI App Initialization
app = FastAPI(
    title="Enhanced Indian Diet Recommendation API",
    description="A backend API for personalized Indian diet suggestions using RAG, nutrition database, and LLMs.",
    version="0.5.0",
)
...
class ChatRequest(BaseModel):
    query: str
    session_id: str = Field(None, description="Optional session ID")
...
async def chat(chat_request: ChatRequest, request: Request):
    ...
    try:
            ...
            try: 
                if tool_name == "handle_greeting":
                    ...
                    if last_ai_message_content:
                        ...
                        # Get relevant nutrition data for context
                        user_params = {
        ...
        else: # Corrected: This now correctly handles the end of the for loop
    ...
    # Detect sentiment and add to session history
...
async def root():
    ...
    return {
        ...
        "usage": "Use POST /chat to interact",
...
@app.get("/nutrition/regional/{region}")
...
if __name__ == "__main__":
    import uvicorn
    logging.info("🚀 Starting Enhanced FastAPI application with Nutrition Database...")
